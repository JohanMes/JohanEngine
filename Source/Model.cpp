#include "Model.h"
#include "Resource.h" // Globals, Utils
#include "Console.h" // Globals, Utils
...
void Model::FreeBuffers() {
	...
	localindexbuffer = NULL;
	Utils::SafeRelease(vertexbuffer);
	vertexbuffer = NULL;
	Utils::SafeRelease(indexbuffer);
	indexbuffer = NULL;
}
void Model::LoadFromOBJ(const char* filename) {
	...
	Utils::ExtractFileName(fullpath,this->filename);
	...
	while(fgets(currentline,sizeof(currentline),modelfile)) {
		if(sscanf(currentline,"%[^ #\r\n]",word1) == 1) {
			if(!strcmp(word1,"v")) {
				...
				int spacecount = Utils::CountChar(Utils::TrimRight(currentline),' ');
...
void Model::SendToGPU() {
	...
	HRESULT result = Globals::d3ddev->CreateVertexBuffer(vertexbuffersize,0,0,D3DPOOL_MANAGED,&vertexbuffer,NULL);
	switch(result) {
	...
	result = Globals::d3ddev->CreateIndexBuffer(indexbuffersize,0,D3DFMT_INDEX32,D3DPOOL_MANAGED,&indexbuffer,NULL);
	switch(result) {
		case D3DERR_INVALIDCALL: {
...
void Model::Print() {
	console->Write("INFO: properties of class Model\r\n");
