...
*/
DLLIMPORT class Renderer* renderer = NULL; // Deze globals zijn overal voor nodig. Niet netjes, maar goed...
...
DLLIMPORT std::vector<float3> ComputeGaussKernel(int size,float sigma2) {
	...
	for(int i = 0;i < (int)result.size();i++) { // float3 = [x y coeff]
		result[i].x = (i % size) - 2;
		result[i].y = (i / size) - 2;
		result[i].z = 1.0f/(2*pi*sigma2) * expf(-(powf(result[i].x - 0,2)/(2 * sigma2) + powf(result[i].y - 0,2)/(2 * sigma2))); // zeros for p0
	}
	return result;
}
