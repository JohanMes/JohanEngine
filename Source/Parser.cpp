...
char* Parser::GetStringValue(const char* text) { // always returns copy, don't forget to free
	Var* variable = GetVariable(text);
	if(variable) {
	...
	if(parent == NULL) {
		if(!strcmp(function,"AddObject")) {
			if(args.size() == 8) { // add from .object file
				...
				float3 pos(GetFloatValue(args[1]),GetFloatValue(args[2]),GetFloatValue(args[3]));
				float3 rot(GetFloatValue(args[4]),GetFloatValue(args[5]),GetFloatValue(args[6]));
				...
				float3 pos(GetFloatValue(args[2]),GetFloatValue(args[3]),GetFloatValue(args[4]));
				float3 rot(GetFloatValue(args[5]),GetFloatValue(args[6]),GetFloatValue(args[7]));
				...
				// Return object itself
				result = new Var(vtObject,"tmp",level);
				result->objectvalue = new Object(name,args[0],args[1],pos,rot,GetFloatValue(args[8]));
			} else if(args.size() == 10) { // add, name provided
				float3 pos(GetFloatValue(args[3]),GetFloatValue(args[4]),GetFloatValue(args[5]));
				float3 rot(GetFloatValue(args[6]),GetFloatValue(args[7]),GetFloatValue(args[8]));
							
				// Return object itself
				result = new Var(vtObject,"tmp",level);
				result->objectvalue = new Object(args[0],args[1],args[2],pos,rot,GetFloatValue(args[9]));
			} else {
			...
			if(args.size() > 5) {
				scene->AddHeightMap(args[0],args[1],GetFloatValue(args[2]),GetFloatValue(args[3]),GetFloatValue(args[4]),GetIntValue(args[5]));
			} else {
			...
			if(args.size() > 5) {
				...
				float3 dir(GetFloatValue(args[0]),GetFloatValue(args[1]),GetFloatValue(args[2]));
				float3 col(GetFloatValue(args[3]),GetFloatValue(args[4]),GetFloatValue(args[5]));
			...
			if(args.size() > 9) {
				...
				float3 pos(GetFloatValue(args[0]),GetFloatValue(args[1]),GetFloatValue(args[2]));
				float3 lat(GetFloatValue(args[3]),GetFloatValue(args[4]),GetFloatValue(args[5])); // look at
				float3 col(GetFloatValue(args[6]),GetFloatValue(args[7]),GetFloatValue(args[8]));
	...
	// object member functions
		...
		if(!strcmp(function,"Translate")) {
			...
			if(args.size() == 1) {
				...
				float4x4 rotation;
				rotation.LookAt(parent->objectvalue->GetTranslation(),pos);
	...
	// path member functions
		...
		if(!strcmp(function,"AddToTail")) {
			...
			if(args.size() > 3) {
				...
				// Return object
				result = new Var(vtObject,"tmp",level);
				result->objectvalue = new Object(name,args[1],args[2],pos,rot,GetFloatValue(args[3]));
			} else {
...
void Parser::GetExpression(std::vector<Token*>& result) {
	...
	if(end + 1 < tokens.size() and (tokens[end + 1]->type == ttMulDiv or tokens[end + 1]->type == ttAddSub)) { // >0 operators
		end++; // start on top of first one
		while(end + 2 < tokens.size() and (tokens[end + 2]->type == ttMulDiv or tokens[end + 2]->type == ttAddSub)) {
			end += 2; // check next
		}
...
void Parser::HandleAssignment(Var* lvalue) { // parent variable
	...
	if(index + 1 < tokens.size() and tokens[index+1]->type == ttOpenParenth) { // C style assignment
		rvalue = HandleFunction(NULL); // steps over ending )
	} else if(index + 3 < tokens.size() and tokens[index + 1]->type == ttMember and tokens[index + 3]->type == ttOpenParenth) { // member function assignment
...
void Parser::Tokenize(const char* file) {
	...
	while(index + 1 < tokens.size()) {
		...
		if(length > 2 and (!strcmp(&token[length-2],"++") or !strcmp(&token[length-2],"--"))) {
	...
	while(index < tokens.size()) {
		...
		} else if(!strcmp(text,"int") or !strcmp(text,"float") or !strcmp(text,"string") or !strcmp(text,"Object") or !strcmp(text,"Path") or !strcmp(text,"float3")) {
			tokens[index]->type = ttVarType;
		} else if(!strcmp(text,"*") or !strcmp(text,"/")) {
			tokens[index]->type = ttMulDiv;
		} else if(!strcmp(text,"+") or !strcmp(text,"-")) {
			tokens[index]->type = ttAddSub;
		} else if(!strcmp(text,"+=") or !strcmp(text,"=") or !strcmp(text,"-=") or !strcmp(text,"++") or !strcmp(text,"--")) {
	...
	while(index < tokens.size()) {
		if(index + 1 < tokens.size() and tokens[index + 1]->type == ttOpenParenth) {
			...
			if(lvalue) {
				...
				} else if(index + 3 < tokens.size() && tokens[index + 1]->type == ttMember && tokens[index + 3]->type == ttOpenParenth) {
					index += 2;
